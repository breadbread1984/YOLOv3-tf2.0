...
def bbox_to_tensor(img_shape, num_classes = 80):
  ...
  level1_coords = tf.keras.layers.Lambda(lambda x, h, w: tf.clip_by_value(tf.cast(tf.reverse(x[..., 0:2], axis = [-1]) * tf.constant([[h // 32, w // 32]], dtype = tf.float32), dtype = tf.int32), clip_value_min = 0, clip_value_max = [[h//32-1, w//32-1]]), arguments = {'h': img_shape[1], 'w': img_shape[0]})(level1_bbox); # level1_coords.shape = (level1_num, 2) in sequence of (h, w)
